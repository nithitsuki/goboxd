...
12. [x] **Per-language seccomp profiles** — shipped 2026-08-20 (additive-merge). New optional per-language `seccomp:` YAML field lists extra deny syscalls (comma/space-separated). seccomp.CombinedWith merges extras into the global deny-list, ALWAYS carrying every global entry (never-weaker invariant, derived policy name/action). A profile is applied via --seccomp_string; languages without one keep --seccomp_policy byte-identical. Mechanism demonstrated by a denied-syscall test (chmod → SIGSYS); ptrace already globally denied so no production profile currently adds value. Verified: exact-set global-coverage test, runtime chmod denial, empty-case byte-identical, graded by dropping a global entry and removing the wiring. M3 shipped 2026-09-04: CombinedWith validates every extra against the kafel amd64 table (generated `validSyscalls` from the vendored kafel + `TestSyscallTableInSync` drift guard + `scripts/gen-seccomp-table.sh`; `SYSCALL[n]` accepted, which also documents why umount2 needs `SYSCALL[166]`). A bad name fails at args construction as Infra → internal_error naming the offender (live-verified: `TestSeccompBadSyscallIsInfraError`), instead of nsjail's ambiguous exit 255 misreading as build_failed/runtime_error.
13. [x] **Mask /proc and /etc/hosts** — shipped 2026-08-20. /proc is mounted per-jail (fresh pid ns, no host processes) via --disable_proc + manual mount; /proc/sys is masked with a tmpfs (no kernel config); /etc/hosts is masked to localhost-only (no container hostname). Residual: /proc/mounts leaks the host mount table (nsjail bind API cannot overlay the symlink target); /proc/self/environ shows the allowlisted jail env. Bounded: jailed uid has no caps, mount/unshare are seccomp-denied, /proc/sys is masked; only cosmetic /proc/self writes (e.g. comm) are possible. Verified: TestJailProcAndHostsMasked (localhost-only hosts, /proc/sys empty count 0), graded by removing each mask. Decision: Option A (rw proc + masked /proc/sys). /proc must stay mounted: Go/Java/Python/Node read /proc/self/* and /proc/meminfo/cpuinfo at runtime (--disable_proc alone breaks them). Option C resolved 2026-09-04: `-m none:/proc:proc:ro` DOES mount proc read-only on kernels >= 6.3 (nsjail's new mount path honors `:ro` via applyGenericMountOption; the legacy path silently drops it, keeping rw proc as documented residual there). Live-verified: effective /proc is ro (comm write fails EROFS), the /proc/sys tmpfs mask stays visible on top (0 entries), reads work, and a Go binary runs clean under it. TestJailProcAndHostsMasked now pins all three (hosts, empty /proc/sys, EROFS + ro mount-table entry, kernel-gated to >= 6.3 since jail and test share the host kernel), graded by dropping `:ro` (comm becomes writable, table shows rw). Deliberately NOT added: nodev/noexec/nosuid on the proc mount — no demonstrated threat beyond what ro already closes, and noexec risks memfd-exec flows; rejected on compat grounds.
...
Dropped entirely (decision 2026-09-04, will not revisit): dragon, pure,
freebasic. Rationale recorded in git history; the registry is closed at
50 languages.
