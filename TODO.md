...
13. [x] **Mask /proc and /etc/hosts** — shipped 2026-08-20. /proc is mounted per-jail (fresh pid ns, no host processes) via --disable_proc + manual mount; /proc/sys is masked with a tmpfs (no kernel config); /etc/hosts is masked to localhost-only (no container hostname). Residual: /proc/mounts leaks the host mount table (nsjail bind API cannot overlay the symlink target); /proc/self/environ shows the allowlisted jail env. Bounded: jailed uid has no caps, mount/unshare are seccomp-denied, /proc/sys is masked. Verified: TestJailProcAndHostsMasked (localhost-only hosts, /proc/sys empty count 0), graded by removing each mask. /proc must stay mounted: Go/Java/Python/Node read /proc/self/* and /proc/meminfo/cpuinfo at runtime (--disable_proc alone breaks them). Option C resolved 2026-09-04: `-m none:/proc:proc:ro` mounts proc read-only on EVERY supported kernel, not just >= 6.3 — nsjail's legacy mount(2) path passes `:ro` as mount data, and the kernel's generic monolithic parser translates it to SB_RDONLY before proc's own option parser runs (vfs_parse_sb_flag, verified in v6.0 source and live on 7.2; a 6.0 host behaves identically to 7.2 here). Pre-fs_context kernels would EINVAL the mandatory mount instead — loud total failure, never silently rw. TestJailProcAndHostsMasked pins ro unconditionally (comm EROFS + ro mount-table entry), graded by dropping `:ro`. Deliberately NOT added: nodev/noexec/nosuid on the proc mount — no demonstrated threat beyond what ro already closes, and noexec risks memfd-exec flows; rejected on compat grounds.
